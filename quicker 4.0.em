    ...
    while(nIdx < iLen)
    {
          ...
          || (szLine[nIdx] == ",") || (szLine[nIdx] == ":") )
        {
        	if(start == 0)
        	{
	        	...
	        	if(Icur <= 0)
	        	{
	       			return strmid(szLine,sIdx,nIdx)
	       		}
	       		start = 1
	       	}
        }
...
macro TrimKeyWord(szLine)
{
	...
	while(1)
	{
		szTmp = GetWord(szLine, nIdx)
		nIdx = nIdx + 1
		if(strlen(szTmp) == 0)
			break
		if(toupper(szTmp) != "STATIC" && toupper(szTmp) != "VOLATILE")
		{
			szRet = cat(szRet, szTmp)
			szRet = cat(szRet, " ")
		}
	}
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 5, " * 版权说明   : Copyright (c) 2008-@szYear@   叶超群")
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if(newFunc != 1)
    {
        ...
        if(strlen(symbol) > 0)
        {
          ...
          //  szRet =  GetWord(szTemp, 1)
          	szRet = TrimKeyWord(szTemp);
    ...
    if(newFunc != 1)
    {
        ...
        while ( i < lnMax) 
        {
            ...
            szParamDescribe = cat(": ",szParamDescribe)
